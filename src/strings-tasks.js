...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (charCode >= 65 && charCode <= 90) {
      const tempCode = charCode + 13;
      newCharCode = tempCode > 90 ? 65 + (tempCode - 91) : tempCode;
    } else if (charCode >= 97 && charCode <= 122) {
      const tempCode = charCode + 13;
      newCharCode = tempCode > 122 ? 97 + (tempCode - 123) : tempCode;
    } else {
...
function getCardId(value) {
  ...
  const titleIndex = titles.indexOf(value.slice(0, value.length - 1));
  const symbolIndex = symbols.indexOf(value.slice(-1));

  return symbolIndex * 13 + titleIndex;
}
